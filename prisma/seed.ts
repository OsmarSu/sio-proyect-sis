...
async function main() {
  ...
  // Borramos en orden inverso a las dependencias de lo que sembramos
  // Clientes
  await prisma.datosCliente.deleteMany();
  await prisma.direccionCliente.deleteMany();
  await prisma.contactoCliente.deleteMany();
  await prisma.cliente.deleteMany();
  await prisma.tipoCliente.deleteMany();
  ...
  // AQUÍ CREAMOS EL LOGIN (password sin hash por compatibilidad con fix-admin.ts)
  ...
  // 6. Crear CLIENTES DEMO (Personas de tipo CLIENTE)
  console.log("👥 Creando clientes demo...");

  const tipoNormal = await prisma.tipoCliente.create({
    data: { nombre: "Normal" },
  });

  const tipoVip = await prisma.tipoCliente.create({
    data: { nombre: "VIP" },
  });

  // Cliente 1 - Normal
  const personaCliente1 = await prisma.persona.create({
    data: {
      nombre: "Juan",
      apellido: "Pérez",
      email: "juan.perez@oasis.com",
      documento: "12345678",
      tipoPersona: "CLIENTE",
    },
  });
  ...
  await prisma.cliente.create({
    data: {
      personaId: personaCliente1.id,
      tipoClienteId: tipoNormal.id,
  ...
  // Cliente 2 - VIP
  const personaCliente2 = await prisma.persona.create({
    data: {
      nombre: "María",
      apellido: "Gómez",
      email: "maria.gomez@oasis.com",
      documento: "87654321",
      tipoPersona: "CLIENTE",
    },
  });
  ...
  await prisma.cliente.create({
    data: {
      personaId: personaCliente2.id,
      tipoClienteId: tipoVip.id,
      fechaRegistro: new Date(),
      estado: true,
      direcciones: {
        create: {
